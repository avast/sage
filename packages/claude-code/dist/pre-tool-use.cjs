...
function normalizeFilePath(raw) {
  ...
  return (0, import_node_path2.normalize)(expanded);
}
...
async function logVerdict(config, sessionId, toolName, toolInput, verdict, userOverride = false) {
  if (!config.enabled)
    return;
...
var VerdictCache = class {
  ...
  async save() {
    if (!this.config.enabled)
      return;
    try {
      await (0, import_promises2.mkdir)((0, import_node_path4.dirname)(this.path), { recursive: true });
...
var RegistryClient = class {
  ...
  parseNpmResponse(name, data, requestedVersion) {
    try {
      ...
      const resolvedVersion = requestedVersion && versions[requestedVersion] ? requestedVersion : latestTag;
  ...
  parsePypiResponse(name, data, requestedVersion) {
    try {
      ...
      const requestedVersionFound = !requestedVersion || Boolean(releases[requestedVersion]?.length);
      const resolvedVersion = requestedVersion && releases[requestedVersion]?.length ? requestedVersion : latestTag;
      let latestHash = "";
...
var DecisionEngine = class {
  ...
  async decide(heuristicMatchesOrSources, urlCheckResults) {
    let sources;
...
async function evaluateToolCall(request, context) {
  ...
  try {
    ...
    if (isAllowlisted(allowlist, request.artifacts)) {
      const verdict2 = allowVerdict("allowlisted");
      await logVerdict(config.logging, request.sessionId, request.toolName, request.toolInput, verdict2, true);
      return verdict2;
    }
  } catch {
  }
  ...
  try {
    await logVerdict(config.logging, request.sessionId, request.toolName, request.toolInput, verdict);
  } catch {
  }
  return verdict;
}
...
async function saveJson(path, data) {
  ...
  await (0, import_promises5.mkdir)((0, import_node_path9.dirname)(resolved), { recursive: true });
  await (0, import_promises5.writeFile)(resolved, `${JSON.stringify(data, null, 2)}
`);
}
...
function formatBlockReason(verdict) {
  ...
  if (isDeny) {
    const lines2 = [" "];
    lines2.push(`${emoji} ${"Threat".padEnd(PAD2)}${reasonText}`);
    appendVerdictDetails(lines2, verdict);
    lines2.push(kv("Action", "Blocked"));
    return lines2.join("\n");
  }
  const header = `\u{1F6E1}\uFE0F Sage by Gen Digital: Suspicious Activity Detected`;
  const lines = [header, separatorLine(SEPARATOR_WIDTH)];
  lines.push(`${emoji} ${"Threat".padEnd(PAD2)}${reasonText}`);
  ...
  if (verdict.decision === "deny") {
    return {
      ...
      hookSpecificOutput: {
        ...
        permissionDecisionReason: "\u{1F6E1}\uFE0F Sage by Gen Digital: Threat Blocked"
      }
    };
  }
  return {
