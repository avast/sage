...
// ---------------------------------------------------------------------------
// PowerShell P/Invoke backend (fallback — persistent process)
// ---------------------------------------------------------------------------

/**
 * Persistent PowerShell script using C# P/Invoke to call the AMSI Win32 API.
 * Compiles the interop type once, initializes AMSI once, then enters a
 * read loop: reads JSON lines from stdin, scans, writes numeric results
 * to stdout. Outputs "READY" once initialization is complete.
 */
...
class PowershellAmsiBackend implements AmsiBackend {
	private readonly logger: Logger;
